...
Background job (objectCountJob.js) — runs via setInterval in PM2 process
  ...
  listCredentials() → for each sub-account (3 at a time, parallel):
    ...
    upsert into object_counts SQLite table
...
## Always use `b2_authorize_account` response URLs — never hard-code hosts

`b2_authorize_account` is the **only** B2 endpoint with a fixed, well-known host.
Every other host must come from its response.

```text
POST https://api.backblazeb2.com/b2api/v4/b2_authorize_account
```

The v4 response shape:
...
|---|---|
| Native B2 API calls (`/b2api/vN/...`) | `body.apiInfo.storageApi.apiUrl` |
| File downloads | `body.apiInfo.storageApi.downloadUrl` |
| S3-compatible API | `body.apiInfo.storageApi.s3ApiUrl` |

**Building an S3 virtual-hosted URL from `s3ApiUrl`:**

```js
const s3Host = new URL(auth.s3ApiUrl).host;
// e.g. "s3.us-west-004.backblazeb2.com"
const host = `${bucketName}.${s3Host}`;
// e.g. "my-bucket.s3.us-west-004.backblazeb2.com"
```

**Extracting the region string for SigV4 signing:**

```js
const region = s3Host.replace(/^s3\./, '').replace(/\.backblazeb2\.com$/, '');
// e.g. "us-west-004"
```

**Why this matters:** Accounts can be assigned to any region (currently US West, US East, EU Central, Canada East — and growing). Hard-coded hosts silently break for accounts outside the assumed region.

**Also avoid** parsing `apiUrl` with a regex to reconstruct the host. Use `new URL(auth.apiUrl).host` instead.
